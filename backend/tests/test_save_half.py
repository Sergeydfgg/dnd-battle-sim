...
def test_save_half_on_success_rounds_down():
    ...
    # damage "1d1+0" => всегда 1; половина = 0
    state, ev = apply_command(
        state,
        SaveEffect(
            source_id="C",
            target_ids=["T"],
            effect_name="burning_hands",
            save_ability="dex",
            dc=1,  # успех
    ...
    assert "EffectDamageRolled" in types
    assert "EffectDamageApplied" in types
    assert state.combatants["T"].hp_current == 10  # урон 0
